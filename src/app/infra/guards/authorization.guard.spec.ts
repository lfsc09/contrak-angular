...
describe('permissionGuard', () => {
    const executeGuard: CanMatchFn = (...guardParameters) => TestBed.runInInjectionContext(() => authorizationGuard(...guardParameters));
    ...
    it('[identity=valid][segments=[]] should return "true" the user have permission', () => {
        identityServiceSpy.identity.and.returnValue({
            roles: new Map<string, null>([['ROUTE_PERMISSION', null]]),
        } as UserIdentity);
        const input = {
            route: {
                data: {
                    role: 'ROUTE_PERMISSION',
                },
            } as Route,
